...
$(document).ready(function(){
    //行内ajax编辑
    $('span[nc_type="microshop_sort"]').inline_edit({act: 'microshop',op: 'update_microshop_personal_sort'});
    //时间
  ...
  <form method="post" id="list_form">
    ...
    <table class="table tb-type2">
      ...
      <tbody>
        ...
        <?php $microshop_url = getMicroshopUrl();?>
        ...
        <tr class="hover edit">
          ...
          <td class="w48 sort"><span nc_type="microshop_sort" column_id="<?php echo $value['personal_id'];?>" title="<?php echo $lang['nc_editable'];?>" class="editable tooltip"><?php echo $value['microshop_sort'];?></span></td>
          <td><?php $personal_image_array = getMicroshopPersonalImageUrl($value,'tiny');?>
            ...
            <a href="<?php echo $microshop_url;?>/index.php?act=personal&op=detail&personal_id=<?php echo $value['personal_id'];?>" style="background:url(<?php echo $personal_image_array[$i];?>) no-repeat center center; width:60px; height:60px; float:left; margin-right:5px;" target="_blank"> 
                <img class="show_image" src="<?php echo TEMPLATES_PATH;?>/images/transparent.gif" style=" width:60px; height:60px; display:block;"/>
            <div class="type-file-preview"> <img  src="<?php echo $personal_image_array_240[$i];?>" /> </div>
            </a>
            <?php } ?></td>
          <td><?php if(empty($microshop_url)) { ?>
            <?php echo $value['personal_id'];?>
            <?php } else { ?>
            <a href="<?php echo $microshop_url.DS.'index.php?act=personal&op=detail&personal_id='.$value['personal_id'];?>" target="_blank"> <?php echo $value['personal_id'];?> </a>
            <?php } ?></td>
          <td><?php if(empty($microshop_url)) { ?>
            <?php echo $value['member_name'];?>
            <?php } else { ?>
            <a href="<?php echo $microshop_url.DS.'index.php?act=home&member_id='.$value['commend_member_id'];?>" target="_blank"> <?php echo $value['member_name'];?> </a>
            <?php } ?></td>
          <td><?php echo $value['commend_message'];?></td>
          <td class="align-center"><?php echo date('Y-m-d',$value['commend_time']);?></td>
          <td class="yes-onoff align-center"><a href="JavaScript:void(0);" class="tooltip <?php echo $value['microshop_commend']? 'enabled':'disabled'?>" ajax_branch='personal_commend'  nc_type="inline_edit" fieldname="microshop_commend" fieldid="<?php echo $value['personal_id']?>" fieldvalue="<?php echo $value['microshop_commend']?'1':'0'?>" title="<?php echo $lang['editable'];?>"><img src="<?php echo TEMPLATES_PATH;?>/images/transparent.gif"></a></td>
          <td class="align-center"><a href="###" onclick="submit_delete(<?php echo $value['personal_id'];?>)"><?php echo $lang['nc_delete'];?></a></td>
        </tr>
        <?php } ?>
        <?php }else { ?>
        <tr class="no_data">
      ...
      <tfoot>
        <tr class="tfoot">
          ...
          <td colspan="16"><label for="checkallBottom"><?php echo $lang['nc_select_all']; ?></label>
            &nbsp;&nbsp;<a href="JavaScript:void(0);" class="btn" onclick="submit_batch();"><span><?php echo $lang['nc_delete'];?></span></a>
