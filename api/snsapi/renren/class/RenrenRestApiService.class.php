...
require (BasePath.DS.'api'.DS.'snsapi'.DS.'renren'.DS.'class'.DS.'config.inc.php');
require_once (BasePath.DS.'api'.DS.'snsapi'.DS.'renren'.DS.'class'.DS.'HttpRequestService.class.php');
...
class RenrenRestApiService extends HttpRequestService{
	...
	public function __construct(){		
		...
		if(empty($this->_configrenren->APIURL) || empty($this->_configrenren->APIKey) || empty($this->_configrenren->SecretKey)){
	...
	public function GET(){
		...
		return $this->_GET($this->_configrenren->APIURL, $this->_params);
	...
	public function rr_post_curl(){
		...
		return $this->_POST($this->_configrenren->APIURL, $this->_params);
	...
	private function generateSignature(){
			...
			$this->_params = $arr;
			$str = md5($str.$this->_configrenren->SecretKey);
	...
	private function setConfigToMapping(){
			$this->_keyMapping['api_key']	= $this->_configrenren->APIKey;
			$this->_keyMapping['method']	= $this->_currentMethod;
			$this->_keyMapping['v']			= $this->_configrenren->APIVersion;
			$this->_keyMapping['format']	= $this->_configrenren->decodeFormat;
		return $this;
	}

	private function setAPIURL($url){
			$this->_configrenren->APIURL = $url;
	}
  ...
  /**
    ...
    */
	...
	public function rr_post_fopen(){
		...
		return $this->_POST_FOPEN($this->_configrenren->APIURL, $this->_params);
	...
	public function rr_photo_post_fopen(){
		...
		unset($args);
		return $this->_photoUpload($this->_configrenren->APIURL, $this->_params,$photo_files);
