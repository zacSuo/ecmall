      ...
      <tbody>
        ...
        <tr class="hover edit <?php echo $val['class_parent_id']=='0'?'':'two';?> <?php echo 'parent'.$val['class_parent_id'];?>">
          <td class="w36"><input type="checkbox"  value="<?php echo $val['class_id'];?>" class="checkitem">
            ...
            <img class="node_parent" state="close" node_id="<?php echo 'parent'.$val['class_id'];?>" src="<?php echo TEMPLATES_PATH;?>/images/tv-expandable.gif">
            <?php } ?></td>
          <td class="w48 sort"><span title="<?php echo $lang['nc_editable'];?>" ajax_branch="class_sort" datatype="number" fieldid="<?php echo $val['class_id'];?>" fieldname="sort" nc_type="inline_edit" class="editable tooltip"><?php echo $val['sort'];?></span></td>
          <td class="name"><?php if($val['class_parent_id'] != '0'){ ?>
            <img fieldid="<?php echo $val['class_id'];?>" status="close" nc_type="flex" src="<?php echo TEMPLATES_PATH;?>/images/tv-item1.gif">
            <?php } ?>
            <span title="<?php echo $lang['nc_editable'];?>" required="1" fieldid="<?php echo $val['class_id'];?>" ajax_branch="class_name" fieldname="class_name" nc_type="inline_edit" class="node_name editable tooltip"><?php echo $val['class_name'];?></span>
