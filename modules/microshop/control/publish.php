...
class publishControl extends MircroShopControl{
    ...
    public function goods_buyOp() {
        ...
        $goods_list = $model_order->getOrderGoodsList($condition,$page);
    ...
    public function goods_saveOp() {
        ...
        $commend_goods_info['commend_goods_price'] = $goods_info['goods_store_price'];
        $commend_goods_info['commend_goods_image'] = $goods_info['goods_image'];
        $commend_goods_info['commend_goods_image_more'] = $goods_info['goods_image_more'];
        ...
        $message = Language::get('nc_common_save_fail');
            ...
            if($result) {
                ...
                if(isset($_POST['share_app_items'])) {
                    ...
                    $commend_goods_info['url'] = MICROSHOP_SITEURL.DS."index.php?act=goods&op=detail&goods_id=".$result;
    ...
    public function personal_image_uploadOp() {
        ...
        if(isset($_SESSION['member_id'])) {
            if(!empty($_FILES['personal_image_ajax']['name'])) {
                ...
                $upload->set('thumb_width',C('thumb_tiny_width').',240');
    ...
    public function personal_image_deleteOp() {
        $data = array();
        $data['status'] = 'success';
        $image_name = BasePath.DS.ATTACH_MICROSHOP.DS.$_SESSION['member_id'].DS.$_GET['image_name'];
    ...
    public function personal_link_addOp() {
        ...
        $result = self::get_goods_info_by_link($link);
        if($result) {
    ...
    public function personal_saveOp() {
        ...
        if(!empty($_POST['personal_buy_link'])) {
            for ($i = 0,$count = count($_POST['personal_buy_link']); $i < $count; $i++) {
                $check_link = self::check_personal_buy_link($_POST['personal_buy_link'][$i]);
                if($check_link) {
        ...
        if($result) {
            ...
            if(isset($_POST['share_app_items'])) {
                ...
                $personal_info['url'] = MICROSHOP_SITEURL.DS."index.php?act=personal&op=detail&personal_id=".$result;
                self::share_app_publish('publish',$personal_info);
            }
        }
        showDialog($message,MICROSHOP_SITEURL.DS.'index.php?act=home&op=personal',$result? 'succ' : 'error','');
    }
