	...
	public function pic_listOp(){
		...
		if(chksubmit()){
			...
			foreach ($ap_list as $val){
				@unlink(BasePath.DS.ATTACH_MALBUM.DS.$val['member_id'].DS.$val['ap_cover']);
				@unlink(BasePath.DS.ATTACH_MALBUM.DS.$val['member_id'].DS.$val['ap_cover'].'_240x240.'.get_image_type($val['ap_cover']));
				@unlink(BasePath.DS.ATTACH_MALBUM.DS.$val['member_id'].DS.$val['ap_cover'].'_max.'.get_image_type($val['ap_cover']));
			}
