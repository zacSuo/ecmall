...
$(document).ready(function(){
    //行内ajax编辑
    $('span[nc_type="microshop_sort"]').inline_edit({act: 'microshop',op: 'update_microshop_goods_sort'});
    //时间
        ...
        <table class="table tb-type2">
            ...
            <tbody>
                ...
                <tr class="hover edit">
                    ...
                    <td class="w48 sort">
                        <span nc_type="microshop_sort" column_id="<?php echo $value['commend_id'];?>" title="<?php echo $lang['nc_editable'];?>" class="editable tooltip"><?php echo $value['microshop_sort'];?></span>
                    </td>
                    <td>
                            <?php $image_url = cthumb($value['commend_goods_image'],'tiny',$value['commend_goods_store_id']);?>
                        <a href="<?php echo $microshop_url.DS.'index.php?act=goods&op=detail&goods_id='.$value['commend_id'];?>" style="background:url(<?php echo $image_url;?>) no-repeat center center; width:60px; height:60px; float:left; margin-right:5px;" target="_blank">
                            ...
                            <div class="type-file-preview">
                                <?php $image_url_240 = cthumb($value['commend_goods_image'],'240x240',$value['commend_goods_store_id']);?>
                                <img  src="<?php echo $image_url_240;?>" title="<?php echo $value['commend_goods_name'];?>" alt="<?php echo $value['commend_goods_name'];?>" />
                            </div>
                        </a>
                    </td>
                    <td>
                        <?php if(empty($microshop_url)) { ?>
                        <?php echo $value['commend_id'];?>
                        <?php } else { ?>
                        <a href="<?php echo $microshop_url.DS.'index.php?act=goods&op=detail&goods_id='.$value['commend_id'];?>" target="_blank">
                            <?php echo $value['commend_id'];?>
                        </a>
                        <?php } ?>
                    </td>
                    <td>
                        ...
                        <?php } else { ?>
                        <a href="<?php echo $microshop_url.DS.'index.php?act=home&member_id='.$value['commend_member_id'];?>" target="_blank">
                            <?php echo $value['member_name'];?>
                        </a>
                        <?php } ?>
                    </td>
                    ...
                    <td class="yes-onoff">
                        <a href="JavaScript:void(0);" class="tooltip <?php echo $value['microshop_commend']? 'enabled':'disabled'?>" ajax_branch='goods_commend'  nc_type="inline_edit" fieldname="microshop_commend" fieldid="<?php echo $value['commend_id']?>" fieldvalue="<?php echo $value['microshop_commend']?'1':'0'?>" title="<?php echo $lang['editable'];?>"><img src="<?php echo TEMPLATES_PATH;?>/images/transparent.gif"></a>
                    </td>
                    <td class="align-center"><a href="###" onclick="submit_delete(<?php echo $value['commend_id'];?>)"><?php echo $lang['nc_delete'];?></a></td>
                </tr>
                <?php } ?>
                <?php }else { ?>
                <tr class="no_data">
