  ...
  <div class="publish-goods-list">
    <ul>
      ...
      <li class="<?php echo in_array($val['goods_id'],$output['commend_goods_array'])?'selected':'select' ?>">
        <div class="picture"><span class="thumb size210"><i></i><a href="<?php echo SiteUrl.DS.ncUrl(array('act'=>'goods','goods_id'=>$val['goods_id']),'goods',$val['store_domain']);?>" target="_blank"> <img src="<?php echo thumb($val,'240x240');?>" onload="javascript:DrawImage(this,210,210);" alt="<?php echo $val['goods_name'];?>" title="<?php echo $val['goods_name'];?>" /> </a></span></div>
        <dl>
          <?php if($output['goods_type'] == 'buy') { ?>
          <dd class="fl"><?php echo $lang['currency'].ncPriceFormat($val['goods_price']);?></dd>
          <dd class="fr"><?php echo $lang['microshop_goods_buy_at'];?>&nbsp;<?php echo date('Y-m-d',$val['add_time']);?></dd>
          <?php } else { ?>
          <dd><?php echo $lang['currency'].ncPriceFormat($val['goods_store_price']);?></dd>
          <?php } ?>
        </dl>
        <div class="recommand-btn">
          ...
          <?php } else { ?>
          <?php if($output['goods_type'] == 'buy') { ?>
          <a href="javascript:void(0)" class="btn_commend_dialog" goods_id="<?php echo $val['goods_id'];?>" goods_name="<?php echo $val['goods_name'];?>" goods_price="<?php echo $lang['currency'].$val['goods_price'];?>" goods_image="<?php echo thumb($val,'small');?>"><?php echo $lang['microshop_goods_commend_want'];?></a>
          <?php } else { ?>
          <a href="javascript:void(0)" class="btn_commend_dialog" goods_id="<?php echo $val['goods_id'];?>" goods_name="<?php echo $val['goods_name'];?>" goods_price="<?php echo $lang['currency'].$val['goods_store_price'];?>" goods_image="<?php echo thumb($val,'small');?>"><?php echo $lang['microshop_goods_commend_want'];?></a>
          <?php } ?>
          <?php } ?>
        </div>
      </li>
...
<form action="<?php echo MICROSHOP_SITEURL;?>/index.php?act=publish&op=goods_save" method="post" id="add_form" class="feededitor">
