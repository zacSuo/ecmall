      ...
      <tbody>
        <tr>
          ...
          <td><a href="javascript:void(0);" id="ncsubmit" class="btn-search tooltip" title="<?php echo $lang['nc_query'];?>">&nbsp;</a>
            ...
            <a class="btns tooltip" href="index.php?act=brand&op=brand" title="<?php echo $lang['nc_cancel_search'];?>"><span><?php echo $lang['nc_cancel_search'];?></span></a>
            <?php }?>
    ...
    <table class="table tb-type2">
      ...
      <tbody>
        ...
        <tr class="hover edit">
          ...
          <td class="sort"><span class="tooltip editable"  nc_type="inline_edit" fieldname="brand_sort" ajax_branch='brand_sort' fieldid="<?php echo $v['brand_id']?>" datatype="pint" maxvalue="255" title="<?php echo $lang['nc_editable'];?>"><?php echo $v['brand_sort']?></span></td>
          <td class="name"><span class="tooltip editable" nc_type="inline_edit" fieldname="brand_name" ajax_branch='brand_name' fieldid="<?php echo $v['brand_id']?>" required="1"  title="<?php echo $lang['nc_editable'];?>"><?php echo $v['brand_name']?></span></td>
          <td class="class"><span class="tooltip editable" nc_type="inline_edit" fieldname="brand_class" ajax_branch='brand_class' fieldid="<?php echo $v['brand_id']?>" required="1"  title="<?php echo $lang['nc_editable'];?>"><?php echo $v['brand_class']?></span></td>
          <td class="picture"><div class="size-88x44"><span class="thumb size-88x44"><i></i><img width="88" height="44" src="<?php if(!empty($v['brand_pic'])){ echo SiteUrl.'/'.ATTACH_BRAND.'/'.$v['brand_pic'];}else{ echo SiteUrl.'/templates/'.TPL_NAME.'/images/default_brand_image.gif';}?>" onload="javascript:DrawImage(this,88,44);" /></span></div></td>
          <td class="align-center yes-onoff"><?php if($v['brand_recommend'] == '0'){ ?>
            <a href="JavaScript:void(0);" class="tooltip disabled" ajax_branch='brand_recommend' nc_type="inline_edit" fieldname="brand_recommend" fieldid="<?php echo $v['brand_id']?>" fieldvalue="0" title="<?php echo $lang['nc_editable'];?>"><img src="<?php echo TEMPLATES_PATH;?>/images/transparent.gif"></a>
            <?php }else{ ?>
            <a href="JavaScript:void(0);" class="tooltip enabled" ajax_branch='brand_recommend' nc_type="inline_edit" fieldname="brand_recommend" fieldid="<?php echo $v['brand_id']?>" fieldvalue="1"  title="<?php echo $lang['nc_editable'];?>"><img src="<?php echo TEMPLATES_PATH;?>/images/transparent.gif"></a>
            <?php } ?></td>
