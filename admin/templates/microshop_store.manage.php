...
$(document).ready(function(){
    //行内ajax编辑
    $('span[nc_type="microshop_sort"]').inline_edit({act: 'microshop',op: 'update_microshop_store_sort'});
});
        ...
        <table class="table tb-type2">
            ...
            <tbody>
                ...
                <tr class="hover edit">
                    ...
                    <td class="w48 sort">
                        <span nc_type="microshop_sort" column_id="<?php echo $value['store_id'];?>" title="<?php echo $lang['nc_editable'];?>" class="editable tooltip"><?php echo $value['microshop_sort'];?></span>
                    </td>
                    <td>
                        <?php if(empty($microshop_url)) { ?>
                            <?php echo $value['store_name'];?>
                        <?php } else { ?>
                        <a href="<?php echo $microshop_url.DS.'index.php?act=store&op=detail&store_id='.$value['microshop_store_id'];?>" target="_blank">
                            <?php echo $value['store_name'];?>
                        </a>
                        <?php } ?>
                    </td>
                    <td>
                        ...
                        <?php } else { ?>
                        <a href="<?php echo $microshop_url.DS.'index.php?act=home&member_id='.$value['member_id'];?>" target="_blank">
                            <?php echo $value['member_name'];?>
                        </a>
                        <?php } ?>
                    </td>
                    ...
                    <td class="yes-onoff">
                        <a href="JavaScript:void(0);" class="tooltip <?php echo $value['microshop_commend']? 'enabled':'disabled'?>" ajax_branch='store_commend'  nc_type="inline_edit" fieldname="microshop_commend" fieldid="<?php echo $value['store_id']?>" fieldvalue="<?php echo $value['microshop_commend']?'1':'0'?>" title="<?php echo $lang['editable'];?>"><img src="<?php echo TEMPLATES_PATH;?>/images/transparent.gif"></a>
                    </td>
                    <td><a href="###" onclick="submit_delete(<?php echo $value['store_id'];?>)"><?php echo $lang['nc_delete'];?></a></td>
                </tr>
                <?php } ?>
                <?php }else { ?>
                <tr class="no_data">
