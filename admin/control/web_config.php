...
class web_configControl extends SystemControl{
	...
	public function web_configOp(){
		...
		$web_list = $model_web_config->getWebList();
	...
	public function code_editOp(){
		...
		if(is_array($code_list) && !empty($code_list)) {
			...
			$parent_goods_class = $model_class->getTreeClassList(2,array('gc_show'=>1));//商品分类父类列表，只取到第二级
			...
			$act_adv_list = $model_web_config->getAdvList("act");//左下角活动广告数据
			Tpl::output('act_adv_list',$act_adv_list);
			$adv_list = $model_web_config->getAdvList("adv");//右下角广告数据
