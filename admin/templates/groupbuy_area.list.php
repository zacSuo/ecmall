      ...
      <tbody>
        ...
        <tr class="hover edit <?php echo $val['area_parent_id']=='0'?'':'two';?> <?php echo 'parent'.$val['area_parent_id'];?>">
          <td class="w36"><input type="checkbox"  value="<?php echo $val['area_id'];?>" class="checkitem">
            ...
            <img class="node_parent" state="close" node_id="<?php echo 'parent'.$val['area_id'];?>" src="<?php echo TEMPLATES_PATH;?>/images/tv-expandable.gif">
            <?php } ?></td>
          <td class="w48 sort"><span title="<?php echo $lang['nc_editable'];?>" ajax_branch="area_sort" datatype="number" fieldid="<?php echo $val['area_id'];?>" fieldname="sort" nc_type="inline_edit" class="editable tooltip"><?php echo $val['area_sort'];?></span></td>
          <td class="name"><?php if($val['deep'] == '1'){ ?>
            <img fieldid="<?php echo $val['area_id'];?>" status="close" nc_type="flex" src="<?php echo TEMPLATES_PATH;?>/images/tv-item1.gif">
            <?php } ?>
            <?php if($val['deep'] == '2'){ ?>
            <img src="templates/images/vertline.gif" class="preimg"> <img fieldid="<?php echo $val['area_id'];?>" status="close" nc_type="flex" src="<?php echo TEMPLATES_PATH;?>/images/tv-expandable1.gif">
            <?php } ?>
            <span title="<?php echo $lang['nc_editable'];?>" required="1" fieldid="<?php echo $val['area_id'];?>" ajax_branch="area_name" fieldname="area_name" nc_type="inline_edit" class="node_name editable tooltip"><?php echo $val['area_name'];?></span>
            <?php if($val['node'] != '1'){ ?>
