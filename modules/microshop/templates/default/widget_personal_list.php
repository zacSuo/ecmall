  ...
  <ul id="pinterest">
    ...
    <li class="item">
      ...
      <div class="picture"> <a href="<?php echo MICROSHOP_SITEURL;?>/index.php?act=personal&op=detail&personal_id=<?php echo $value['personal_id'];?>" target="_blank">
        ...
        <?php if($width > 238) {$height = $height*238/$width;$width=238;} ?>
        <img class="lazy" height="<?php echo $height;?>" width="<?php echo $width;?>" src="<?php echo MICROSHOP_TEMPLATES_PATH;?>/images/loading.gif" data-src="<?php echo $personal_image_array[0];?>" /> </a> </div>
      <div class="handle"> <span class="like-btn"><a nc_type="microshop_like" like_id="<?php echo $value['personal_id'];?>" href="javascript:void(0)"><i class="pngFix"></i><span><?php echo $lang['microshop_text_like'];?></span><em><?php echo $value['like_count']<=999?$value['like_count']:'999+';?></em></a></span> <span class="comment"><a href="<?php echo MICROSHOP_SITEURL;?>/index.php?act=personal&op=detail&personal_id=<?php echo $value['personal_id'];?>" target="_blank"><i title="<?php echo $lang['microshop_text_comment'];?>" class="pngFix">&nbsp;</i><em><?php echo $value['comment_count']<=999?$value['comment_count']:'999+';?></em></a></span> </div>
      <dl class="commentary">
        <dt><span class="thumb size30"><i></i><a href="<?php echo MICROSHOP_SITEURL;?>/index.php?act=home&member_id=<?php echo $value['commend_member_id'];?>" target="_blank"> <img src="<?php echo getMemberAvatar($value['member_avatar']);?>" alt="<?php echo $value['member_name'];?>" onload="javascript:DrawImage(this,30,30);" /> </a></span></dt>
        <dd>
          <h4><a href="<?php echo MICROSHOP_SITEURL;?>/index.php?act=home&member_id=<?php echo $value['commend_member_id'];?>" target="_blank"> <?php echo $value['member_name'];?> </a> </h4>
          <p> <a href="<?php echo MICROSHOP_SITEURL;?>/index.php?act=personal&op=detail&personal_id=<?php echo $value['personal_id'];?>" target="_blank"> <?php echo $value['commend_message'];?> </a> </p>
        </dd>
      </dl>
    </li>
    <?php } ?>
...
<script type="text/javascript" src="<?php echo MICROSHOP_RESOURCE_PATH;?>/js/fileupload/jquery.iframe-transport.js" charset="utf-8"></script> 
<script type="text/javascript" src="<?php echo MICROSHOP_RESOURCE_PATH;?>/js/fileupload/jquery.ui.widget.js" charset="utf-8"></script> 
<script type="text/javascript" src="<?php echo MICROSHOP_RESOURCE_PATH;?>/js/fileupload/jquery.fileupload.js" charset="utf-8"></script> 
<!-- 发布个人秀 --> 
<script type="text/javascript">
$(document).ready(function(){
    ...
    $("#fileupload").fileupload({
        dataType: 'json',
            url: "<?php echo MICROSHOP_SITEURL.DS.'index.php?act=publish&op=personal_image_upload';?>",
            add: function(e,data) {
                ...
                if(result.status == "success") {
                    $("#_personal_image_list img").attr("src","<?php echo SiteUrl.DS.ATTACH_MICROSHOP.DS.$_SESSION['member_id'].DS;?>"+result.file);
    ...
    $("[nc_type='btn_personal_image_delete']").live('click',function(){
        ...
        $.getJSON("<?php echo MICROSHOP_SITEURL.DS.'index.php?act=publish&op=personal_image_delete';?>", { image_name: image_name }, function(json){
        });
    ...
    $("#btn_personal_link_add").click(function() {
        ...
        if(link != '') {
            $.getJSON("<?php echo MICROSHOP_SITEURL.DS.'index.php?act=publish&op=personal_link_add';?>", { link: link}, function(data){
                if(data.result == "true") {
...
<div id="publish_div" style="display: none;">
  <form action="<?php echo MICROSHOP_SITEURL;?>/index.php?act=publish&op=personal_save" method="post" id="add_form" class="publish-div">
