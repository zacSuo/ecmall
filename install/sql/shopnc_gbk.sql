...
) ENGINE=MyISAM  DEFAULT CHARSET=gbk COMMENT='投诉主题表';
...
CREATE TABLE `#__rec_position` (
  ...
  PRIMARY KEY  (`rec_id`)
) ENGINE=MyISAM  DEFAULT CHARSET=gbk COMMENT='推荐位';
...
CREATE TABLE `#__sns_membertag` (
  ...
  PRIMARY KEY  (`mtag_id`)
) ENGINE=MyISAM DEFAULT CHARSET=gbk COMMENT='会员标签表';
...
) ENGINE=MyISAM DEFAULT CHARSET=gbk COMMENT='会员标签会员对照表';

CREATE TABLE `#__sns_setting` (
  `member_id` int(11) NOT NULL COMMENT '会员id',
  `setting_skin` varchar(50) default NULL COMMENT '皮肤',
  PRIMARY KEY  (`member_id`)
) ENGINE=MyISAM DEFAULT CHARSET=gbk COMMENT='个人中心设置表';
...
) ENGINE=MyISAM DEFAULT CHARSET=gbk COMMENT='店铺自动发布动态设置表';
...
) ENGINE=MyISAM DEFAULT CHARSET=gbk COMMENT='店铺动态评论表';
...
) ENGINE=MyISAM DEFAULT CHARSET=gbk COMMENT='店铺动态表';
...
CREATE TABLE `#__store_theme` (
  ...
  KEY `store_id` (`store_id`)
) ENGINE=MyISAM  DEFAULT CHARSET=gbk COMMENT='店铺主题表';
