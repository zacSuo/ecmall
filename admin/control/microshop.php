...
class microshopControl extends SystemControl{
    ...
    const MICROSHOP_CLASS_LIST = 'index.php?act=microshop&op=goods_class_list';
    const GOODS_FLAG = 1;
	...
	public function manage_saveOp() {
        ...
        $update_array['microshop_url'] = trim($_POST['microshop_url']);
        ...
        if(!empty($_FILES['microshop_logo']['name'])) {
            ...
            $old_image[] = BasePath.DS.ATTACH_MICROSHOP.DS.C('microshop_logo');
        }
        if(!empty($_FILES['microshop_header_pic']['name'])) {
            ...
            $old_image[] = BasePath.DS.ATTACH_MICROSHOP.DS.C('microshop_header_pic');
        }
     ...
     * 微商城商品(随心看)分类管理
     **/
    public function goods_class_listOp() {
     ...
     * 微商城商品(随心看)分类管理
     **/
    public function personal_class_listOp() {
     ...
     * 微商城个人秀分类添加
     **/
    public function personal_class_addOp() {
     ...
     * 微商城商品(随心看)分类编辑
     **/
    public function personal_class_editOp() {
     ...
     * 微商城个人秀分类保存
     **/
    public function personal_class_saveOp() {
    ...
    private function class_save($type) {
        ...
        if(!empty($_FILES['class_image']['name'])) {
            ...
            if(!empty($_POST['old_class_image'])) {
                $old_image = BasePath.DS.ATTACH_MICROSHOP.DS.$_POST['old_class_image'];
        ...
        if($result) {
            showMessage(Language::get('class_add_success'),"index.php?act=microshop&op={$type}_class_list");
        } else {
            showMessage(Language::get('class_add_fail'),"index.php?act=microshop&op={$type}_class_list",'','error');
        }
     ...
     * ajax修改分类排序
     */
    public function update_goods_class_sortOp() {
        $this->update_class_sort('goods');
    }
    public function update_personal_class_sortOp() {
        $this->update_class_sort('personal');
    }
    private function update_class_sort($type) {
        if(intval($_GET['id']) <= 0) {
            echo json_encode(array('result'=>FALSE,'message'=>Language::get('wrong_argument')));
            die;
        }
     ...
     * ajax修改分类名称
     */
    public function update_goods_class_nameOp() {
        $this->update_class_name('goods');
    }
    public function update_personal_class_nameOp() {
     ...
     * 随心看分类删除
     **/
     public function goods_class_dropOp() {
        ...
        if(!empty($list)) {
            foreach ($list as $value) {
                if(!empty($value['class_image'])) {
                    //删除老图片
                    $old_image = BasePath.DS.ATTACH_MICROSHOP.DS.$value['class_image'];
                    if(is_file($old_image)) {
                        unlink($old_image);
                    }
                }
     ...
     * 个人秀分类删除
     **/
     public function personal_class_dropOp() {
     ...
     * 分类关键字和商品分类的绑定
     **/
    public function goods_class_bindingOp() {
        ...
        $goods_class_list = ($g = F('goods_class')) ? $g : H('goods_class',true,'file');
     ...
     * 分类关键字和商品分类的绑定保存
     **/
    public function goods_class_binding_saveOp() {
     ...
     * 设为默认分类
     **/
    public function goods_class_defaultOp() {
        $class_id = intval($_GET['class_id']);
        if($class_id <= 0) {
            showMessage(Language::get('wrong_argument'),'','','error');
        }
    ...
    public function get_shop_goods_classOp() {
        ...
        if(!empty($list)) {
            foreach ($list as $personal_info) {
                ...
                foreach ($image_array as $value) {
                    //删除原始图片
                    $image_name = BasePath.DS.ATTACH_MICROSHOP.DS.$personal_info['commend_member_id'].DS.$value;
                    ...
                    $ext = explode('.', $value);
                    $ext = $ext[count($ext) - 1];
                    $image_name = BasePath.DS.ATTACH_MICROSHOP.DS.$personal_info['commend_member_id'].DS.$value.'_list.'.$ext;
                    if(is_file($image_name)) {
                        unlink($image_name);
                    }
                    $image_name = BasePath.DS.ATTACH_MICROSHOP.DS.$personal_info['commend_member_id'].DS.$value.'_tiny.'.$ext;
                    if(is_file($image_name)) {
                        unlink($image_name);
                    }
                }
	...
	public function store_addOp(){
        ...
        //获取店铺附加基本信息
        $store_list = $model_store->getStoreInfoBasic($store_list);
		Tpl::output('store_list',$store_list);
     ...
     * 更新微商城店铺排序
     */
    public function update_microshop_store_sortOp() {
     ...
     * 更新微商城随心看排序
     */
    public function update_microshop_goods_sortOp() {
     ...
     * 更新微商城个人秀排序
     */
    public function update_microshop_personal_sortOp() {
    ...
    public function adv_saveOp() {
        ...
        if(!empty($_FILES['adv_image']['name'])) {
            ...
            if(!empty($_POST['old_adv_image'])) {
                $old_image = BasePath.DS.ATTACH_MICROSHOP.DS.'adv'.DS.$_POST['old_adv_image'];
                if(is_file($old_image)) {
                    unlink($old_image);
                }
            }
    ...
    public function adv_dropOp() {
        ...
        if(!empty($list)) {
            foreach ($list as $adv_info) {
                //删除原始图片
                $image_name = BasePath.DS.ATTACH_MICROSHOP.DS.'adv'.DS.$adv_info['adv_image'];
                if(is_file($image_name)) {
                    unlink($image_name);
                }
            }
     ...
     * 广告排序
     */
    public function update_adv_sortOp() {
        if(intval($_GET['id']) <= 0) {
            echo json_encode(array('result'=>FALSE,'message'=>Language::get('wrong_argument')));
            die;
        }
    ...
    private function show_menu_goods_class($menu_key) {
        $menu_array = array(
            'goods_class_list'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_manage'),'menu_url'=>'index.php?act=microshop&op=goods_class_list'),
            'goods_class_add'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_add'),'menu_url'=>'index.php?act=microshop&op=goods_class_add'),
        );
    ...
    private function show_menu_personal_class($menu_key) {
        $menu_array = array(
            'personal_class_list'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_manage'),'menu_url'=>'index.php?act=microshop&op=personal_class_list'),
            'personal_class_add'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_add'),'menu_url'=>'index.php?act=microshop&op=personal_class_add'),
        );
    ...
    private function show_menu_store($menu_key) {
        $menu_array = array(
            ...
            'store_add'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_add'),'menu_url'=>'index.php?act=microshop&op=store_add'),
        );
        $menu_array[$menu_key]['menu_type'] = 'text';
        Tpl::output('menu',$menu_array);
    }	
    ...
    private function show_menu_adv($menu_key) {
        $menu_array = array(
            ...
            'adv_add'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_add'),'menu_url'=>'index.php?act=microshop&op=adv_add'),
        );
